...
- No chips shown after "Review items" on an empty box — the message "This box has no items logged yet. Add some!" appears but no chips are set, leaving the user with no affordance to continue. Fix: call `setBoxOpenChips()` after showing the empty message in `reviewBox`.
