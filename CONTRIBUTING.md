...
- Storage budget counter — display a live countdown of remaining item capacity in the UI, starting at ~14,400 and adjusting after every save based on actual `JSON.stringify(state).length`. Show remaining capacity in items only (at ~200 bytes each). When a photo is added, the counter drops dramatically (~400 items per photo) making the storage cost viscerally clear without requiring the user to understand bytes. Counter should update after every `saveState()` call. Display location TBD — context bar or sidebar header are candidates.
