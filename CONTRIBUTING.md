...
- Rename `chipClick` to reflect its actual purpose — `chipClick(text)` is a general command executor, not chip-specific. It echoes text as a user message and calls `processInput`. Used by chips, fate labels, and other clickable elements. Better name: `runCommand(text)` or `sendMessage(text)`. Implementation approach: create the new function with the correct name, make `chipClick` an alias to it (`const chipClick = runCommand`), then gradually migrate call sites during future refactoring passes. Don't rename all existing implementations at once — this is a background cleanup task, not urgent. The alias preserves backward compatibility while making new code clearer.
...
- Compound command history — multi-step exchanges (e.g. `move` then `bedroom`) should be stored as a single history entry rather than two separate ones. Implemented and reverted — the feature worked mechanically but introduced several UX issues: (1) the combined entry drops the box name context (`move ssss` instead of `move aaaa to ssss`); (2) elliptical chip labels (e.g. `Dump into...`) pollute the combined string; (3) destructive confirmation flows (`delete box` + `yes`) combine into a single replayable string that bypasses the confirmation step when recalled; (4) requires structured command storage rather than joined strings to be safe. Needs design work before reimplementation — likely requires storing commands as `{verb, subject, target}` objects rather than raw strings
...
- Clickable common words in progress stats — the "Common in names" and "Common in notes" sections currently show detected patterns (e.g. "cable", "charger", "broken", "screen") as plain text. Make each word clickable to drill down into items containing that word. Clicking "cable" could show a filtered review list of all items with "cable" in their name, or trigger a search/filter flow. Design questions to resolve: Should it filter to just that word, or show a review screen? Should it preserve fate context (e.g. "show me all trash items with 'cable'")? Should it work across all boxes or just the active box? The feature could reuse existing review flows with word-based filtering, or introduce a new "search items" capability. Consider how this relates to potential future features like global item search or tag-based organization.
