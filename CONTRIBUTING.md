...
- Multi-line item entry — in the add item flow, each line of input should be treated as a separate item entry using the comma-separated format (`name, fate, notes`). Entry point: split submitted text on newlines before passing to `handleItemName`, processing each line in sequence. The textarea should support Shift+Enter to insert a newline (desktop), with Enter alone submitting. On mobile the keyboard return key inserts a newline by default. The user message display should show line breaks as actual newlines (not collapsed to a single line) so "AAA, keep, notes ↵ BBB, keep, notes" is legible. When multiple items are submitted, the confirmation message should summarise all of them ("3 items added") rather than one message per item.
